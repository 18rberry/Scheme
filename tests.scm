...
; BEGIN PROBLEM 0
(* 395 (+ 380 9) (/ 48 84))
; expect 87802.85714285714

(quote (define x quoted))
; expect (define x quoted)

((lambda (a b) (- a b)) 4 2)
; expect 2

(define x '(cons 2 (cons 3 nil)))
; expect x

x
; expect (cons 2 (cons 3 ()))

(define a (cons 2 (cons (* 3 4) nil)))
; expect a

a
; expect (2 12)

(define x `(cons 2 (cons 3 ,(* 8 (- 4 5)))))
; expect x

x
; expect (cons 2 (cons 3 -8))

(begin (define b '(1 2 3 4 5)) (define c (cons 29 b)) (* 200 (car c)))
; expect 5800

(begin (define m 5) (define n 6) ((lambda (x y z) (list x y z)) m n (* m n)))
; expect (5 6 30)

(define (radians x) (/ (* x 3.14) 180))
; expect radians

(define (rad_list lst) (map (lambda (var) (radians var)) lst))
; expect rad_list

(rad_list (list 1 2 3 4))
; expect (0.017444444444444446 0.03488888888888889 0.052333333333333336 0.06977777777777779)
...
(or wrong (and wrong (radians 77) #t) (or nil (/ 1 0) 6))
; expect ()

(let ((better 'llamas)
      (best 'llamasinpajamas)
      (two 100)
      (too 50))
      (cond (wrong 'llamasnotinpajamas)
      ((= two too) 'llamaswearingredpajamas)
      (a best)
      (else better)))
; expect llamasinpajamas


(let ((who '(who is there))
    (whom '(whom is there)))
(begin (display '(knock knock))
(cond (wrong whom)
      (else (display who)))
      (print '(stanford. stanford who?))))
; expect (knock knock)(who is there)(stanford. stanford who?)

(define func (mu () (list n a p)))
; expect func

(begin (define n 'naps) (define a 'are) (define p 'great)
(define func2 (lambda() (func))))
; expect func2

(func2)
; expect (naps are great)
