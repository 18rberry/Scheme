...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if not isinstance(first, Pair) and isinstance(env.lookup(first), MacroProcedure):
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil:
            if formals.__len__() == vals.__len__():
                ...
                raise SchemeError('incorrect number of vals given')
        return new_frame
        # END PROBLEM 11
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('wrong number of arguments were passed', error)
...
def do_and_form(expressions, env):
    ...
    if scheme_falsep(value) or expressions.second == nil:
        return value
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    "*** YOUR CODE HERE ***"
    lst_of_symbols = Pair(bindings.first.first, nil)
    lst_of_expr = Pair(scheme_eval(bindings.first.second.first, env), nil)
    return_symbol = lst_of_symbols
    return_expr = lst_of_expr
    check_form(bindings.first, 2, 2)
    bindings = bindings.second
    while bindings is not nil:
        check_form(bindings.first, 2, 2)
        lst_of_symbols.second = Pair(bindings.first.first, nil)
        lst_of_expr.second = Pair(scheme_eval(bindings.first.second.first, env), nil)
        lst_of_symbols = lst_of_symbols.second
        lst_of_expr = lst_of_expr.second
        bindings = bindings.second
    check_formals(return_symbol)

    return env.make_child_frame(return_symbol, return_expr)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    check_form(expressions.second, 1)
    formal = target.second
    function = expressions.second
    macro = MacroProcedure(formal, function, env)
